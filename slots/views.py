...
from .services.slot_services import (create_slot_service, delete_slot_service,
...
class SlotCreateView(APIView):
    ...
    @extend_schema(request=SlotSerializer)
    def post(self, request):
        serializer = SlotSerializer(data=request.data)
        if serializer.is_valid():
            return create_slot_service(serializer.validated_data)
...
class ChargerSlotsView(APIView):
    ...
    def get(self, request, charger_id):
        return get_charger_slots_service(charger_id)
...
class SlotUpdateView(APIView):
    ...
    def put(self, request, slot_id):
        ...
        if serializer.is_valid():
            return update_slot_service(slot, serializer.validated_data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
