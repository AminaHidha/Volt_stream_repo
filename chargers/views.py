...
from .services.charger_services import create_charger_service


class ChargerCreateView(APIView):
    permission_classes = [IsAuthenticated]

    @extend_schema(request=ChargerSerializer)
    def post(self, request):
        serializer = ChargerSerializer(data=request.data)
        if serializer.is_valid():
            return create_charger_service(serializer.validated_data)
...
class ChargerUpdateView(APIView):
    ...
    def put(self, request, charger_id):
        ...
        serializer = ChargerSerializer(charger, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
            return Response({"message": "Charger updated", "data": serializer.data})
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class ChargerDeleteView(APIView):
    ...
    def delete(self, request, charger_id):
        ...
        charger.delete()
        return Response({"message": "Charger deleted"})
...
class ChargerStatusUpdateView(APIView):
    ...
    def patch(self, request, charger_id):
        ...
        charger.save()
        return Response({"message": "Status updated", "status": charger.status})
