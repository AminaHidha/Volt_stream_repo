...
def send_otp_sms(phone, otp_code, purpose):
    try:
        ...
        # ✅ Auto add +91 if not already there
...
def resend_otp(email, purpose='register'):
    ...
    if old_otp and old_otp.resend_count >= 5:
        return False, 'Maximum resend limit reached. Please try again later'
    ...
    if old_otp and old_otp.last_resend_at:
        ...
        if cooldown.total_seconds() < 30:
            ...
            return False, f'Please wait {seconds_left} seconds before resending'
    ...
    print(f"RESEND — USER PHONE: {user.phone}")
    print(f"RESEND — OTP CODE: {otp_code}")

    # ✅ Send via SMS
    if user.phone:
        print("RESEND — SENDING VIA SMS")
        sms_sent = send_otp_sms(user.phone, otp_code, purpose)
        if not sms_sent:
            print("RESEND — SMS FAILED, FALLING BACK TO EMAIL")
            send_otp_email(user.email, otp_code, purpose)
    else:
        print("RESEND — NO PHONE, SENDING VIA EMAIL")
        send_otp_email(user.email, otp_code, purpose)
